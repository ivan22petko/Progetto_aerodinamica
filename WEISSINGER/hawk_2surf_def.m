...
config.RootChord = [2.65, 1.317 ];
config.DihedralAngle = [0, -10]; % [°]
config.SweepAngle = [30, 35]; % [°]
config.TaperRatio = [0.339622, 0.4403948]; 
% config.AspectRatio = [0]; 
config.Span = [9.39, 4.39];
config.LEPosition_X = [0, 5.395065];  
config.LEPosition_Y = [0, 0];
config.LEPosition_Z = [0, 1.1444]; 
...
CD_hawk_polar_wingtail = zeros(6, 1);
CL_hawk_polar_wingtail = zeros(6, 1);
check = 1;
...
U_Inf = [cosd(beta)*cosd(alpha(alpha_index)) sind(beta)*cosd(alpha(alpha_index)) sind(alpha(alpha_index))] .* U_Inf_Mag;
    ...
    CD_hawk_polar_wingtail(check) = (D3D_wingtail(check, 1) + D3D_wingtail(check, 2))/(0.5*rho*U_Inf_Mag^2*config.Surface(1));
    CL_hawk_polar_wingtail(check) = (L3D_wingtail(check, 1) + L3D_wingtail(check, 2))/(0.5*rho*U_Inf_Mag^2*config.Surface(1));

    check = check + 1;

end

save('CD_hawk_polar_wingtail.mat', 'CD_hawk_polar_wingtail')
save('CL_hawk_polar_wingtail.mat', 'CL_hawk_polar_wingtail')
%% CL/alpha e polare 
